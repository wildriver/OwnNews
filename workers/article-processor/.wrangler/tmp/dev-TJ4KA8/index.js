...
// .wrangler/tmp/bundle-AopvWG/checked-fetch.js
...
// .wrangler/tmp/bundle-AopvWG/strip-cf-connecting-ip-header.js
...
var src_default = {
  async scheduled(event, env, ctx) {
    ...
    const { data: articles, error } = await supabase.from("articles").select("*").is("embedding_m3", null).limit(50);
    if (error) {
    ...
    const { error: updateError } = await supabase.from("articles").upsert(updates, { onConflict: "id" });
    if (updateError) {
...
// .wrangler/tmp/bundle-AopvWG/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-AopvWG/middleware-loader.entry.ts
