...
v = あなたの関心ベクトル / e = 記事のベクトル / α = 学習率`}</code></pre>
                ...
                <section className="space-y-3">
                    ...
                    <ul className="text-[15px] leading-relaxed text-muted-foreground space-y-2 list-disc pl-5">
                        ...
                        <li>
                            <strong className="text-foreground">キーワード検索</strong>: サイドバーの検索は、端末にキャッシュ済みの
                            記事パックをその場で全文一致検索します（検索語はサーバーに送信されません）。
                            検索した回数だけは匿名の利用統計として記録します（検索語そのものは送信・記録されません）。
                            検索したキーワードはそのままウォッチタグとして購読できます。
                        </li>
                        <li>
                            <strong className="text-foreground">話題のキーワード</strong>: TF-IDFと同じ発想で、
                            <strong className="text-foreground">リフト（直近24時間の出現率 ÷ それ以前の平常時の出現率）×
                            注目度（その語を含む記事の閲覧数・リアクション、対数）</strong>で選びます。
                            訃報や株価のように毎日一定量出る語はリフト≒1倍になるため、2倍未満は除外します
                            （「今日特有」かつ「みんなが読んでいる」言葉だけが残る）。
                            ランキングではないため、表示順は毎回シャッフルされます。この計算も端末内で行います。
                        </li>
                        <li>
                ...
                <section className="space-y-3">
                    ...
                    <ul className="text-[15px] leading-relaxed text-muted-foreground space-y-2 list-disc pl-5">
                        ...
                        <li>
                            <strong className="text-foreground">「もっと知る」でX・はてなブックマークを開いた記録は推薦学習に使いません。</strong>
                            どの記事でSNSの反応を見に行ったか（開いた事実）は端末とアカウントに記録されますが、
                            AIで深掘りしたとき（deep_dive）とは別扱いで、関心ベクトルの学習には反映しません。
                            運営が観測するのは「もっと知る」を使った匿名の回数だけです。
                        </li>
                        <li>
